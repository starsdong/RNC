...
void personpower_LHC(int config = 0)
{
  style();
  ...
  const Double_t frac_all[NC][NP][NN][2] = {
    // default
    {{{0., 0.7}, {8., 0.7}, {20., 0.7}, {40, 0.7}, {40, 0.7}},
     {{0., 1.7}, {8., 1.7}, {20., 2.7}, {40, 3.7}, {40, 3.7}},
     {{0., 7.3}, {8., 7.9}, {20., 9.7}, {40, 10.7}, {40, 10.7}}},					
    // minimum
    {{{0., 0.7}, {8., 0.7}, {20., 0.7}, {40, 0.7}, {40, 0.7}},
     {{0., 0.7}, {8., 0.7}, {20., 0.7}, {40, 0.7}, {40, 0.7}},
     {{0., 6.7}, {8., 6.7}, {20., 6.7}, {40, 6.7}, {40, 6.7}}}					
  };
  ...
  for(int i=0;i<NL;i++) {
    ...
    drawText(XP[i]+offset, -1, Form("FY%d",year), 42, 0.06);
  }

  drawText(14, 4.0, "ALICE", 52, 0.06);
  if(config==0) drawText(32, 1.5, "ALICE3", 52, 0.06);
  drawText(33, 0.1, "EIC", 52, 0.06);

  c1->Update();
  c1->SaveAs(Form("personpower_LHC_%s.pdf",NameC[config]));
  c1->SaveAs(Form("personpower_LHC_%s.png",NameC[config]));
