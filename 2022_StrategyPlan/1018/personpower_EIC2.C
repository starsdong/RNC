...
void personpower_EIC2(int config = 0)
{
  style();
  ...
  const Double_t frac_all[NC][NP][NN][2] = {
    ...
    {{{0., 2.0}, {8., 0.8}, {20., 0.8}, {40, 0.8}, {40, 0.8}},
     ...
     {{0., 11.5}, {8., 14.3}, {20., 19.8}, {40, 20.8}, {40, 20.8}}},					
    // CEplus
    {{{0., 2.0}, {8., 0.8}, {20., 0.8}, {40, 0.8}, {40, 0.8}},
     {{0., 2.2}, {8., 1.2}, {20., 2.4}, {40, 3.2}, {40, 3.2}},
     {{0., 5.0}, {8., 5.4}, {20., 3.2}, {40, 7.8}, {40, 7.8}},
     {{0., 12.8}, {8., 13.0}, {20., 18.5}, {40, 19.5}, {40, 19.5}}},					
    // CE
  ...
  c1->Update();
  c1->SaveAs(Form("personpower_EIC2_%s.pdf",NameC[config]));
  c1->SaveAs(Form("personpower_EIC2_%s.png",NameC[config]));

  Double_t yAve[NP][3];
  const Int_t aveEdge[3] = {8, 20, 40};
  std::ofstream outData;
  for(int i=0;i<NP;i++) {
    outData.open(Form("data2/%s_%s.txt",Name[i],NameC[config]));
