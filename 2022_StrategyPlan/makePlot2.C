...
void makePlot2(int config=0)
{
  ...
  const Int_t NP = 6;
  const Char_t *Name[NP] = {"JLab", "Instr-Comp", "ALICE", "STAR", "sPHENIX", "EIC-Other"};
  const Char_t *LabelName[NP] = { "JLab", "Instr-Comp", "ALICE", "STAR", "sPHENIX", "EIC"};
  ...
  c1->SaveAs(Form("FTE_CE_Check2_%d.pdf",config));
  c1->SaveAs(Form("FTE_CE_Check2_%d.png",config));
