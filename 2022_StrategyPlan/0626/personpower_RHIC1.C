...
void personpower_RHIC1()
{
  ...
  // };
  Double_t frac[NP][NN][2] = {{{2., 0.6}, {14., 0.8}, {30., 1.6}},
  			      {{2., 0.6}, {14., 1.1}, {30., 2.1}},
  			      {{2., 2.6}, {14., 4.0}, {30., 2.7}},
  			      {{2., 3.5}, {14., 1.1}, {30., 0.6}}
  ...
  for(int j=0;j<NN;j++) {
    for(int i=1;i<NP;i++) {
      frac[i][j][1] += frac[i-1][j][1];
    }
  }
  for(int i=0;i<NP;i++) {
    cout << Name[i] << "\t" << frac[i][0][1] << "\t" << frac[i][1][1] << "\t" << frac[i][2][1] << endl;
  }
...
//  c1->SetTopMargin(0.001);
  ...
  for(int i=0;i<NL;i++) {
    ...
    drawText(XP[i]+offset, -0.8, Form("FY%d",year), 42, 0.06);
  }

  drawText(2, 5.0, "STAR", 52, 0.06);
  drawText(12, 4.0, "sPHENIX", 52, 0.06);
  drawText(32, 3.0, "CBM", 52, 0.06);
  drawText(33, 0.8, "EIC", 52, 0.06);

  c1->Update();
  c1->SaveAs("personpower_RHIC1.pdf");
  c1->SaveAs("personpower_RHIC1.png");

  Double_t yAve[NP][3];
  const Int_t aveEdge[3] = {8, 20, 40};
  std::ofstream outData;
  for(int i=0;i<NP;i++) {
    outData.open(Form("data1/%s.txt",Name[i]));
