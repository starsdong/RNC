...
void personpower(const int config = 0, const double CEoff = 2.3)
{
  style();
  ...
  const Char_t *EICName[4] = {"EIC1","EIC2","EIC3","EIC_instr"};
  ...
  for(int i=0;i<NP;i++) {
	...
	for(int j=0;j<N+1;j++) yy[i][j] += CEoff;	
      }
	...
	for(int j=0;j<N+1;j++) {
      ...
      for(int j=0;j<N+1;j++) {
	yy_EIC_diff[j] = yy[i][j] - yy_EIC_0[j];
      }
  ...
  const Double_t xPos[NC][NP] = { {2, 12, 32, 16, 34, 12, 34},
				  {2, 12, 32, 16, 34, 12, 34},
				  {1, 12, 32, 16, 34, 12, 34}};
  const Double_t yPos[NC][NP] = { {19.4, 18.6, 18.6, 12.4, 12.8, 8.0, 3.0},
				  {19.4, 18.6, 18.6, 12.4, 12.8, 8.0, 3.0},
				  {24.2, 27.6, 26.6, 22.0, 20.8, 17.6, 6.0}};
  for(int i=0;i<NP;i++) {
    double yt = yPos[config][i];
    if(config==0 || config==1) yt += CEoff;
    drawText(xPos[config][i], yt, Name[NP-i-1], 52, 0.055);  
  }
  
  const Double_t CE = 21.7+CEoff;  // current total: from spreadsheet - Jim
  drawLine(x1, CE, x2, CE, 3, 9, 1);
  drawLine(x1, 0, x2, 0, 3, 9, 1);

  c1->Update();
  c1->SaveAs(Form("personpower_%d.pdf",config));
  c1->SaveAs(Form("personpower_%d.png",config));
